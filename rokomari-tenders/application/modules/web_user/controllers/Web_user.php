...
class Web_user extends Backend_Controller
{
    ...
    public function dashboard($id=null)
    {
        ...
        if($is_valid_pkg)
        {
            ...
            $ts_user_cat_list=$this->Common_model->get_data_by('ts_user_cat_list', 'ucat_user_id', $userid);
            ...
            if ($id==="add_more") {
                ...
                $this->data['subview'] = 'select_tenders';
            }else {
                if (empty($ts_user_cat_list)) {
                    $this->data['all_inviters'] = $this->Common_model->get_data('ts_inviters');
        ...
        else
        {
            ...
            if (!empty($user_pkg->upkg_user_id)) {
                if ($user_pkg->upkg_user_id==$userid) {
                    $this->data['pkg_message'] = 'Your package duration is expired. Please Renew to continue.';
                    $this->data['pkg_message_color'] = 'danger';
                    $this->data['all_pkg'] = $this->Common_model->get_data('ts_packages', 'pkg_price', 'ASC');

                    $user = $this->db->select('*')->from('ts_web_user')->where('webu_id', $userid)->get()->result_array();
                    $pkg_free = $this->db->select('*')->from('ts_packages')->where('pkg_price', 0)->get()->result_array();
                    $this->data['pkg_duration'] = $pkg_free[0]['pkg_duration'];
                    ...
                    $day_count=round($datediff / (60 * 60 * 24));
                    $this->data['duration'] = $day_count;
                    
                    $this->data['subview'] = 'select_package'; 
                }
                // custom_var_dump($this->data['all_pkg']);
            }
            else {
                $this->data['pkg_message'] = 'You are not on a package. Please select a package to continue';
                $this->data['pkg_message_color'] = 'secondaty';
                $this->data['all_pkg'] = $this->Common_model->get_data('ts_packages', 'pkg_price', 'ASC');
    ...
    public function add_user_products()
    {
        ...
        $user_sub_cat_name = $this->input->post('ids_array');
    ...
    public function profile(){
       $user= $this->data['user_info']= $this->Common_model->user_info();
        $userid = $this->session->userdata('user_id');
        $save_tender=$this->db->select('save_tender_id')->from('ts_user_fav_ten_list')->where('ufav_user_id', $userid)->get()->result();
        // custom_var_dump($save_tender);
        
       
        if (!empty($save_tender) && $save_tender[0]->save_tender_id != '[]') {
            $save_tender_id=$save_tender[0]->save_tender_id;
            $get_save_tender=$this->Common_model->get_fav_tender(json_decode($save_tender_id));
            $this->data['all_save_tenders'] = $get_save_tender;
        }
        ...
        $this->data['all_tenders']= $this->Common_model->get_data('ts_packages', 'pkg_price', 'ASC');
        $this->data['user_pck']= $this->Common_model->get_tender_product_count();
