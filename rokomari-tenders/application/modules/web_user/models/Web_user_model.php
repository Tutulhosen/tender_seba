...
class Web_user_model extends CI_Model {
    ...
    */
    ...
    public function get_user_selected_product_count()
    {
        $user_id=$this->session->userdata('user_id');

        $this->db->select ('*');
        $this->db->from('ts_sub_categories');
        $this->db->join('ts_user_cat_list', 'ts_sub_categories.sub_cat_id = ts_user_cat_list.ucat_sub_cat_id');
        $this->db->where('ts_user_cat_list.ucat_user_id', $user_id);
        return $this->db->get()->result();
    }
}
