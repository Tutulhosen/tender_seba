...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('webu_status', 1)->users()->row();  // active = webu_status 04-04-18
      // $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->email->to($user->webu_email);
					// $this->email->to($user->email); webu_email = email 04-04-18
	...
	/**
	 * is_invoice_generator
	 * 30-10-17
	 * @return bool
	 * @author Ben Edmunds
	 **/
	public function is_invoice_generator($id=false)
	{
		$this->ion_auth_model->trigger_events('is_invoice_generator');

		$invoice_group = $this->config->item('invoice_group', 'ion_auth');

		return $this->in_group($invoice_group, $id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_stock_manager($id=false)
	{
		$this->ion_auth_model->trigger_events('is_stock_manager');

		$stock_group = $this->config->item('stock_group', 'ion_auth');

		return $this->in_group($stock_group, $id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_setup_user($id=false)
	{
		$this->ion_auth_model->trigger_events('is_setup_user');

		$setup_group = $this->config->item('setup_group', 'ion_auth');

		return $this->in_group($setup_group, $id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_default_user($id=false)
	{
		$this->ion_auth_model->trigger_events('is_default_user');
