...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$data = array(
			    ...
			    'webu_status'          => 1		//25-02-18 webu_status
			);

			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, array('webu_id' => $id));	//25-02-18
		}
		else
		{
			$data = array(
	...
	public function deactivate($id = NULL)
	{
		...
		$data = array(
		    ...
		    'webu_status'          => 0		//25-02-18
		);

		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('webu_id' => $id));//webu_id 25-02-18
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('webu_id, password, salt')	//08-04-18
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('webu_id', 'desc')	//id => webu_id //08-04-18
		                  ->get($this->tables['users']);
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    // 'username'   => $identity,	//comment out at 19-02-18
		    'password'   => $password,
		    'webu_email' => $email,	//change email to webu_email from email
		    'ip_address' => $ip_address,
		    'created_on' => time(),
		    'webu_created' => date('Y-m-d H:i:s'),
		    'webu_status'     => ($manual_activation === false ? 1 : 0)	//change active to webu_status
		);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', webu_id, password, webu_status, last_login, created_on')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('webu_id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->webu_id, $password);	//20-02-18 webu_id

			if ($password === TRUE)
			{
				if ($user->webu_status == 0)	//20-02-18 active = webu_status
				{
					$this->trigger_events('post_login_unsuccessful');
				...
				$this->update_last_login($user->webu_id);	//20-02-18 webu_id
	...
	public function users($groups = NULL)
	{
		...
		{
			//default selects 	//25-02-18 webu_id
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.webu_id as id',
			    $this->tables['users'].'.webu_id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.webu_id',
				    'inner'
				);
			}
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['users'].'.webu_id', 'desc');	//25-02-18 webu_id
		$this->where($this->tables['users'].'.webu_id', $id);	//25-02-18 webu_id
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('webu_id' => $id));	//20-02-18 webu_id
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->webu_email,	//20-02-18	email = webu_email
		    // 'first_name'		   => $user->webu_full_name, //20-02-18 first_name = webu_full_name
		    // 'last_name'		   	   => $user->last_name,
		    'user_id'              => $user->webu_id, //everyone likes to overwrite id so we'll use user_id
